...
def check_magic_numbers(code: str) -> List[str]:
    ...
    conditional_pattern = r"(?:IF|ELSIF)\s+.*?[><!=]+\s*(-?\d+(?:\.\d+)?)"
    matches = re.findall(conditional_pattern, code, re.IGNORECASE)
...
def validate_program_structure(code: str) -> Dict:
    ...
    # Check order: PROGRAM -> VAR -> BEGIN -> END_PROGRAM
    sections = []
    
    if re.search(r"PROGRAM\s+\w+", code, re.IGNORECASE):
        sections.append("PROGRAM")
    
    if re.search(r"VAR.*END_VAR", code, re.DOTALL | re.IGNORECASE):
        sections.append("VAR")
    
    if re.search(r"BEGIN", code, re.IGNORECASE):
        sections.append("BEGIN")
    
    if re.search(r"END_PROGRAM", code, re.IGNORECASE):
        sections.append("END_PROGRAM")
    
    expected_order = ["PROGRAM", "VAR", "BEGIN", "END_PROGRAM"]
    
    # Check if sections are in correct order
    section_positions = []
    for section in expected_order:
        match = re.search(rf"\b{section}\b", code, re.IGNORECASE)
        if match:
    ...
    for i in range(1, len(section_positions)):
        if section_positions[i][1] <= section_positions[i-1][1]:
            errors.append(f"Structure Error: {section_positions[i][0]} appears before {section_positions[i-1][0]}")
    
    return {
        "valid": len(errors) == 0,
        "errors": errors,
        "sections_found": [s[0] for s in section_positions]
    }
